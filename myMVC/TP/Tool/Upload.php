...
class Upload{
    ...
    public function up(	$namespace){

        if( !isset(	$_FILES[$name]) ){//判断是否有文件上传

            throw new \Exception(	'没有文件上传',500	);
        ...
        if( !$this->isAllowExt( $this->getExt() ) ){
            ...
            throw new \Exception('请选择jpeg,jpg,gif,bmp,png格式文件',500);

        }
        //判断文件大小
        if( !$this->isAllowSize( $this->file['size']) ){

            throw new \Exception('文件超出限制',500);
    ...
    public function createDir(){
        ...
        if(  is_dir($dir) || mkdir($dir,0777,true) ){
            ...
            throw new \Exception('创建文件失败',500);
